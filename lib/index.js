...
var libfs       = require('fs'),
    ...
    libwmap     = require('weakmap'),
    MESSAGES = {
        'unknown bundle': 'Unknown bundle "{bundle}"',
        'unknown config': 'Unknown config "{config}" in bundle "{bundle}"',
        'unknown cached data': 'Unknown cache data with config "{config}" in bundle "{bundle}"',
 ...
 *
 * The default implementation uses the Node module `weakmap` as a cache,
 * which can use the ES6 WeakMap implementation if available.
 *
...
Config.prototype._setConfigCache = function (bundleName, configName, context, config, hasMerge) {
    ...
    return new libpromise.Promise(function (fulfill, reject) {
        ...
        cache      = configPath[mergeName] = (configPath[mergeName] || new WeakMap());

        cache.set(context, config);

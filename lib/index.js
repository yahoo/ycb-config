...
Config.prototype.read = function (bundleName, configName, context) {
    ...
    return new libpromise.Promise(function (fulfill, reject) {
        bundleCache = self._configCache[bundleName];
        if (!bundleCache) {
            reject(new Error(MESSAGES['unknown bundle']));
            return;
        }

        configCache = bundleCache[configName];
        if (!configCache) {
            reject(new Error(MESSAGES['unknown config']));
            return;
        }

        config = configCache[JSON.stringify(context)];
        if (config) {
            fulfill(config); 
        }
    }).then(null, function () {
        return self._getYCB(bundleName, configName).then(function (ycb) {
            var config = ycb.read(context, {}),
                path = [bundleName, configName, JSON.stringify(context)],
                i, len = path.length;

            for (i = 0; i < len; i++) {
                if (!self._configCache[path[i]]) {
                    self._configCache[path[i]] = {};
                }
            }

            self._configCache[path[i]] = config;

...
Config.prototype._getConfigCache = function (bundleName, configName, context, hasMerge) {
    ...
    if (!bundlePath) {
        return Promise.reject(new Error(liblang.sub(MESSAGES['unknown bundle'], {bundle: bundleName})));
    }

    configPath = bundlePath[configName];
    if (!configPath) {
        return Promise.reject(new Error(liblang.sub(MESSAGES['unknown config'], {bundle: bundleName, config: configName})));
    }

    mergePath = configPath[mergeName];
    if (!mergePath) {
        return Promise.reject(new Error(liblang.sub(MESSAGES['unknown cache data'], {bundle: bundleName, config: configName})));
    }

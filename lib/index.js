...
Config.prototype.read = function (bundleName, configName, context) {
    var self = this;
    return this.readNoMerge(bundleName, configName, context).then(function (sections) {
        var out = {};
        if (sections.length === 1) {
            return sections[0];
        }
        sections.forEach(function (section) {
            out = merge(section, out);
        });
        return out;
    });
};
...
Config.prototype.readNoMerge = function (bundleName, configName, context) {
    ...
    return new libpromise.Promise(function (fulfill, reject) {
        ...
        if (self._configYCBs[path]) {
            fulfill(self._configYCBs[path].readNoMerge(self._expandContext(context), {}));
            return;
        }
        ...
        return self._readConfigContents(path).then(function (c) {
            ...
            if (isYCB) {
                try {
                    ...
                    fulfill(ycb.readNoMerge(self._expandContext(context), {}));
                } catch (err) {
                    reject(err);
                }
                return;
